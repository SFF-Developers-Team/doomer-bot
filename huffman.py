...
class Huffman:
    # Static frequency table where the index (0-255) is the ASCII value
    _huffman_frq = [
    ...
    _huffman_tree: Optional[Dict] = None
    _huffman_table: Dict[int, str] = {}

    @staticmethod
    def build_binary_tree() -> None:
        """Build the Huffman binary tree based on frequency table"""
        if Huffman._huffman_tree is not None:
            return

        # Create starting leaves
        tree = []
        for i in range(256):
            tree.append({"frq": Huffman._huffman_frq[i], "asc": i})

        # Pair leaves and branches based on frequency until single root remains
        for _ in range(255):
            lowest_key1 = -1
            lowest_key2 = -1
            lowest_frq1 = 1e30
            ...
            for j in range(256):
                ...
                if tree[j]['frq'] < lowest_frq1:
                    lowest_key2 = lowest_key1
                    lowest_frq2 = lowest_frq1
                    lowest_key1 = j
                    lowest_frq1 = tree[j]['frq']
                elif tree[j]['frq'] < lowest_frq2:
                    lowest_key2 = j
                    lowest_frq2 = tree[j]['frq']

            # Join the two together under a new branch
            tree[lowest_key1] = {
                "frq": (lowest_frq1 + lowest_frq2),
                "branch0": tree[lowest_key2],
                "branch1": tree[lowest_key1]
            }
            tree[lowest_key2] = False

        Huffman._huffman_tree = tree[lowest_key1]

    @staticmethod
    def binary_tree_to_lookup_table(branch: Optional[Dict] = None, binary_path: str = "") -> None:
        """Convert binary tree to lookup table"""
        if branch is None:
            if Huffman._huffman_table:
                return
            branch = Huffman._huffman_tree
        ...
        Huffman._huffman_table[branch['asc']] = binary_path

    @staticmethod
    def encode(data: bytes) -> bytes:
        """Encode data using Huffman coding"""
        Huffman.build_binary_tree()
        Huffman.binary_tree_to_lookup_table()

        binary_string = ""
        for byte in data:
            ascii_val = byte
            binary_path = Huffman._huffman_table[ascii_val]
            binary_string += binary_path

        # Store padding length in first byte
        padding_value = 8 - (len(binary_string) % 8)
        padding_bits = f"{padding_value:08b}"[::-1]  # Little-endian
        binary_string = padding_bits + binary_string
        ...
        for i in range(0, len(binary_string), 8):
            ...
            if len(chunk) < 8:
                chunk += '0' * (8 - len(chunk))
            byte = int(chunk[::-1], 2)  # Little-endian
    ...
    def decode(data: bytes) -> bytes:
        ...
        # Get padding length from first byte
        ...
        binary_string = ""
        for byte in data:
            binary_string += f"{byte:08b}"[::-1]  # Little-endian
        ...
        while binary_string:
            matched = False
            for ascii_val, binary_path in Huffman._huffman_table.items():
                path_len = len(binary_path)
                if binary_string.startswith(binary_path):
                    decoded_bytes.append(ascii_val)
                    binary_string = binary_string[path_len:]
                    matched = True
                    break
